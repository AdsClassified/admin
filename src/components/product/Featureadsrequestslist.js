...
// import { makeFeatureAd } from '../../Connection/Ads';
import { getFeatureRequestsImages } from '../../Connection/Ads';

const Featureadsrequestslist = ({
  handleDelete,
  // handleEdit,
  handleReject,
  handleApprove,
  ...
  const fetchImages = async adId => {
    let res = await getFeatureRequestsImages({ adId: adId });
    console.log(res);
  ...
  const handleFeature = async (type, data) => {
    ...
    if (type === 'accept') {
      let res = await makeFeatureAd({
        ...data,
        type: type,
        featureAd: true,
        featureAdReviewed: true,
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Accept Feature</TableCell>
                  <TableCell>Reject Feature</TableCell>
                </TableRow>
              </TableHead>
              {ads.length > 0 && (
                <TableBody>
                  {ads.map(ad => (
                    ...
                    >
                      ...
                      <TableCell>
                        <button
                          className="btn btn-warning m-2"
                          onClick={() => handleFeature('accept', ad)}
                        >
...
Featureadsrequestslist.propTypes = {
  ads: PropTypes.array.isRequired
};

export default Featureadsrequestslist;
